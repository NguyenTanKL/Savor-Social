...
function ProfilePage() {
  ...
  const { posts, loading: isLoading, error } = useSelector(state => state.posts || {});  const [type, setType] = useState("posts");
  const [isFollowing, setIsFollowing] = useState(false);
  const [user, setUser] = useState({});
  const { userId } = useParams();
  const dispatch = useDispatch();
  ...
  // Lọc bài viết theo userId
  const userPosts = (postState && Array.isArray(postState))
  ? postState.filter((post) => post.userId && post.userId.toString() === userId)
  : [];
  console.log("userPosts:",userPosts);
  const handlePostDelete = () => {
    // Không cần gọi lại getPostsAsync vì state đã được cập nhật trong postSlice
